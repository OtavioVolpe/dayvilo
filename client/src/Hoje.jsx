...
import { Plus, Star, Clock, ListTodo } from 'lucide-react';
...
export default function Hoje() {
  ...
  async function adicionar(evento) {
    ...
    try {
      ...
      const perfil = await solicitar('/perfil');
      if (perfil.data_hoje !== data) {
        definirTentativa(valor => valor + 1);
        throw new Error('Um novo dia começou. Confira sua lista e salve a tarefa novamente.');
      }
      const { tarefa } = await solicitar('/tarefas', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ titulo: campos.get('titulo'), horario: campos.get('horario') || null,
          observacao: campos.get('observacao'), prioridade: campos.has('prioridade'), data_prevista: data }),
      });
      definirTarefas(anteriores => [...anteriores, tarefa]);
      formulario.reset(); definirAberto(false);
  ...
  const linha = tarefa => (
    <li className={`tarefa ${tarefa.situacao === 'concluida' ? 'concluida' : ''}`} key={tarefa.id}>
      <input type="checkbox" aria-label={`Concluir: ${tarefa.titulo}`} checked={tarefa.situacao === 'concluida'} disabled={atualizando.includes(tarefa.id)} onChange={() => alternar(tarefa)} />
  ...
  return <section aria-label="Tarefas de hoje" aria-busy={carregando}>
    <div className="lista-cabecalho"><h2>Minha rotina</h2><button className="botao-principal" disabled={carregando || !data} onClick={() => definirAberto(!aberto)} aria-expanded={aberto}><Plus size={17} aria-hidden="true" />Nova tarefa</button></div>
    ...
    <form onSubmit={adicionar} className="formulario-tarefa" hidden={!aberto}>
      <fieldset disabled={salvando || carregando}><legend>Nova tarefa para hoje</legend>
        <label>O que você quer fazer?<input name="titulo" required maxLength={200} placeholder="Ex.: ler algumas páginas" /></label>
        <div className="campos-opcionais"><label>Horário (opcional)<input name="horario" type="time" /></label><label className="campo-prioridade"><input name="prioridade" type="checkbox" />Marcar como prioridade</label></div>
        <label>Observação (opcional)<textarea name="observacao" maxLength={4000} rows={2} /></label>
        <div className="acoes-formulario"><button type="button" className="botao-secundario" onClick={() => definirAberto(false)}>Cancelar</button><button className="botao-principal" type="submit">{salvando ? 'Salvando…' : 'Salvar tarefa'}</button></div>
      </fieldset>
    </form>
